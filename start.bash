...
if findscreen xvfb; then
	...
	screen -dm -S xvfb Xvfb $DISPLAY -screen 0 480x360x24 +extension RANDR -ac
	sleep 1
fi
...
if findscreen ffmpeg; then
	...
	screen -dm -S ffmpeg bash -c "
		...
		ffmpeg -f x11grab \
			-i $DISPLAY \
			-vf "drawtext=textfile='overlay.txt':x=300:y=0:fontcolor=white@0.5:fontsize=12:reload=1" \
			-f $SIXEL_ENCODER \
			-s $RESOLUTION \
			-diffuse $DIFFUSE \
			-reqcolors $COLORS \
